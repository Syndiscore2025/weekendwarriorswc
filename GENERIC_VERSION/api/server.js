...
const CONTACT_NOTIFICATION_EMAIL = process.env.CONTACT_NOTIFICATION_EMAIL || 'contact@yourclub.com';
const REGISTRATION_NOTIFICATION_EMAIL = process.env.REGISTRATION_NOTIFICATION_EMAIL || 'registrations@yourclub.com';
const VERIFIED_SENDER_EMAIL = process.env.VERIFIED_SENDER_EMAIL || 'noreply@yourclub.com';
...
const REPO_OWNER = process.env.REPO_OWNER || 'YourGitHubUsername';
const REPO_NAME = process.env.REPO_NAME || 'yourrepo';
...
// Helper function to update files in GitHub
async function updateGitHubFile(path, content, message) {
  try {
    ...
    try {
      ...
      if (error.status === 404) {
        console.log(`File doesn't exist, will create new file: ${path}`);
      } else {
        throw error;
      }
    }
    ...
    if (file === 'data/contact-messages.json') {
      try {
        ...
        if (messages.length > 0) {
          ...
          const subject = '📧 New Contact Form Submission - Your Wrestling Club';
          const htmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p style="color: #888; font-size: 0.9em; margin-top: 30px;">
                View all messages in the <a href="https://www.yourclub.com/admin.html">admin dashboard</a>
              </p>
            </div>
          `;
          const textContent = `
...
View all messages in the admin dashboard: https://www.yourclub.com/admin.html
          `;
    ...
    if (file === 'data/winter-signups.json') {
      try {
        ...
        if (signups.length > 0) {
          ...
          const subject = '🤼 New Wrestler Registration - Your Wrestling Club';
          const htmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #4db8ff; border-bottom: 3px solid #4db8ff; padding-bottom: 10px;">
              ...
              <p style="color: #888; font-size: 0.9em; margin-top: 30px;">
                View all registrations in the <a href="https://www.yourclub.com/admin.html">admin dashboard</a>
              </p>
            </div>
          `;
          const textContent = `
...
View all registrations in the admin dashboard: https://www.yourclub.com/admin.html
          `;
